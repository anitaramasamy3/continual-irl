...
class Net(torch.nn.Module):
    ...
    def forward(self, x, t):
        ...
        for cc in range(offset1, offset2):
            # import pdb; pdb.set_trace()
            means[cc - offset1] = self.net(Variable(self.mem_class_x.items()[cc][1])).data.mean(0)
    ...
    def observe(self, x, t, y):
        ...
        if self.num_exemplars > 0:
            ...
            for tt in range(t):
                ...
                inp_dist = inp_dist.to(device)
                target_dist = target_dist.to(device)
                for cc in range(self.nc_per_task):
                    indx = random.randint(0, self.num_exemplars - 1)
                    # import pdb; pdb.set_trace()
                    inp_dist[cc] = self.mem_class_x.items()[cc + offset1][1][indx].clone()
                    target_dist[cc] = self.mem_class_y.items()[cc + offset1][1][indx].clone()
                # Add distillation loss
                # import pdb; pdb.set_trace()
                # print ('before loss', loss)
                loss += self.reg * self.kl(self.lsm(self.net(Variable(inp_dist))[:, offset1: offset2]),self.sm(Variable(target_dist[:, offset1: offset2]))) * self.nc_per_task
        ...
        if self.examples_seen == self.samples_per_task:
            ...
            if self.gpu:
                all_labs = torch.LongTensor(np.unique(self.memy.cpu().numpy())).to(device)
            else:
                all_labs = torch.LongTensor(np.unique(self.memy.numpy())).to(device)
