...
from nn_tf import Policy_Network
...
def train_net(model, params, weights, path, trainFrames, i=10):
    ...
    while t < train_frames:
        ...
        # new_s = np.array([[1,0,0,1,0,1,0,0,1,0,0,0]])
        ...
        if t > observe:
            ...
            # history = LossHistory()
            actions, steploss, kl, ms= model.train(X_train, y_train)
            if t%1000 == 0:
                print(steploss, kl, ms)
        ...
        # print(t) 
        if t % train_frames == 0:
            model.save_weights('saved-models_'+ path +'/evaluatedPolicies/'+str(i)+'-'+ filename + '-' + str(t) + '.h5')
...
def IRL_helper(weights, path, trainFrames, i,model):
    nn_param = [164, 150]
    params = {
        "batchSize": 100,
        "buffer": 50000,
    ...
    # model = Policy_Network(NUM_INPUT, nn_param, sess, i)
