...
class irlAgent:
    def __init__(self, randomFE, expertFE, epsilon, num_states, num_frames, behavior):
        ...
        self.brown_model = Policy_Network(self.num_states, [164, 150], self.sess, '',saved_model,None, False)
        self.model = Policy_Network(self.num_states, [164, 150], self.sess, 'red',None,self.brown_model, True)
        init = tf.global_variables_initializer()
        self.sess.run(init)
        # xtrain = np.load('xtrain_brown.npy')
        # ytrain = np.load('ytrain_brown.npy')
        # self.model.compute_fisher(xtrain, ytrain,250)
        # self.model.star()
        # self.model.update_ewc_loss(0.5)
